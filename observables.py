...
lightRed = (255/255.0, 152/255.0, 150/255.0)
...
def plotEnergy():
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111, ylim=[-7.0,10.0])
    ...
    left, bottom, width, height = [0.6,0.63, 0.3, 0.29]
    ax2 = fig.add_axes([left, bottom, width, height])
    ax2.plot(t, temp, lw=3, color=orange)
    ...
    ax2.set_ylabel('Temp ($\epsilon /k_B$)')
    ax2.set_ylim([1.3,1.8])

    fig.tight_layout()
    #fig.savefig('./Results/EnTempNVE.pdf')
    plt.show()
    return
...
def plotMSD():
    ...
    ax.plot(t, msd, lw='4', color=blue, label='Surrounding Particles') #: slope={0:.3f}'.format(m))
    ...
    ax.plot(t, msd_col5, lw='2', color=red, alpha=1.0)
...
def spatialDensity():
    ...
    #dens = np.loadtxt('restart_pos.txt')
    ...
    ax.axvspan(-0.5, 4, color='lightBlue', alpha=0.5, label='cold region')
    ax.axvspan(4, 8.5, color=lightRed, alpha=0.5, label='hot region')
